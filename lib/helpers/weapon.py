...
def build_weapon_skill_lua(skill_id, skill_patch, language, lang="en"):
    ...
    for i, match in enumerate(placeholder_matches):
        ...
        for rank_vals in blackboards:
            if "*" in key:
                parts = key.split("*")
                v = rank_vals.get(parts[0].strip(), 0.0) * rank_vals.get(parts[1].strip(), 0.0)
            elif "/" in key:
                parts = key.split("/")
                v1 = rank_vals.get(parts[0].strip(), 0.0)
                v2 = rank_vals.get(parts[1].strip(), 0.0)
...
### Depreciated since cargo got hooked up
